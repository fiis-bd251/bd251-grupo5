...
# Índice
  1. [Sobre el módulo](#sobre-el-módulo)

      1.1. [Responsabilidades](#responsabilidades)

      1.2. [Flujo de trabajo](#flujo-de-trabajo)
      
      1.2. [Diagrama BPMN](#diagrama-bpmn)

  2. [Ejemplos de interacciones](#ejemplos-de-interacciones)
  3. [Interacción Control de Entrega - Reclamo](#interacción-control-de-entrega---reclamo)
  4. [Interacción Control de Entrega - Recepción de órdenes](#interacción-control-de-entrega---recepción-de-órdenes)
  5. [Interacción Control de Entrega - Monitoreo en Ruta](#interacción-control-de-entrega---monitoreo-en-ruta)
...
Para la visualización completa de las interacciones y el flujo de trabajo del módulo:

![Diagrama BPMN](bpmn_modulo6.png)

### **Ejemplos de interacciones**
### **Interacción Control de Entrega - Reclamo**

  **Paso 1.** En el momento en el que el conductor presiona el botón de "enviar" en la pestaña de Incidencia, enviaremos un JSON al módulo de Reclamos con los detalles de esta, ya que nosotros no nos encargamos de acuñar un id de incidencia o reclamo.

  **Política: Toda incidencia o reclamo debe venir de Gestión de reclamos. Prohibido generar IDs locales.**
  ...
  **Paso 2.** Gestión de Reclamos genera su propio id_reclamo en base a los datos anteriormente enviados.

    {
      "status": "reclamo_creado",
      "id_reclamo": "RCL-0056",
      "url_reclamo": "http://localhost:8080/reclamos/RCL-0056"
    }

  **Paso 3.** Control de entrega almacena solo la referencia (no modifica el reclamo)

    UPDATE entregas 
    SET id_reclamo = 'RCL-0056'  -- Solo FK, no hace falta ningún dato más
    WHERE id_entrega = 'ENT-001';

  **Ventajas**:
  - Obtenemos trazabilidad ya que el ID de reclamo en Control de Entrega es de **solo lectura**.
  - Centralizamos la autoridad de IDs y evitamos conflictos futuros de cara al despliege del sistema con alto volumen de entregas.
  
### Interacción Control de Entrega - Recepción de órdenes
### Interacción Control de Entrega - Monitoreo en Ruta
